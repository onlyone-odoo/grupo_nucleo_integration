...
* La API de GN solo expone GetCatalog (no hay endpoint separado para precio/stock). El sync completo del catálogo funciona así:
  ...
  * **Grupo Núcleo: Lotes de sync precio y stock** — corre **cada 5 minutos** solo cuando el sync precio/stock fue solicitado. Solo actualiza productos existentes: supplierinfo (precio), *Stock Grupo Núcleo*, dimensiones. No crea productos. Al completar el ciclo, realiza cleanup automático: productos GN previamente sincronizados que ya no aparecen en el catálogo de la API (stock=0 según docs) tienen ``stock_gn`` seteado a 0 y se recalcula el costo de reposición (para el glue dual-vendor con ELIT). Al completar, el cron de cleanup lo desactiva.
  ...
  * Publicar/despublicar productos por stock: no incluido en el módulo (lógica específica por cliente). Recomendamos crear una acción de servidor o cron custom que use `search([('is_gruponucleo_product', '=', True), ('stock_gn', '>', umbral)])` para publicar y similar para despublicar, basado en el campo `stock_gn` y `gn_publish_stock_threshold`.
...
* **17.0.2.0.0** — Odoo 17. Sync catálogo y precio/stock por lotes (trigger + batch + cleanup), dimensiones Zippin, descripciones venta/eCommerce, venta sin stock, cleanup automático de stock_gn=0 para productos ausentes del catálogo API (solo retorna stock positivo), health check de API cada hora con indicador en Ajustes y notificación por Discuss, costo de reposición desde supplierinfo vía ``product_replenishment_cost``.
